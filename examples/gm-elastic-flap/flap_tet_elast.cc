...
// class BodyForce
    ...
    std::vector<Vector<double>> &  value_list) const
  {
   ...
   get_stress_strain_tensor<dim>(/*lambda = */ 84.0e9,
                                  /*mu     = */ 84.0e9);
...
tetElast<dim>::tetElast(
  ...
  : triangulation(MPI_COMM_WORLD)
    ...
    , quadrature_formula(fe.degree + 1)  // so, 4?
...
void tetElast<dim>::nodal_bcs()
{
	...
	const double nrad = 0.001;  // Need a better way to make this.  Maybe tie to bcpoints[0].norm()
...
void tetElast<dim>::setup_system()
{
  ...
  unsigned int f101 = 0;
  unsigned int f102 = 0;
//  Point<dim> ringct(376.0, -524.0, 851.0);
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    ...
    for (const auto &face : cell->face_iterators())
       ...
       if (face->at_boundary())
           {
             //if (std::fabs(face->center()[1] + 449.4)   < 1e-2)

//    	   if (face-> center().distance(ringct)  < 5.0)
    	   if (face->center()[2] > 1237.0)
             {
               face->set_boundary_id(101);
               f101 += 1;
             }
             if (face->center()[2]  < 730.0 )
             {
  ...
  pcout << "Fixed Faces assigned to 101 = " << f101 << std::endl;
  pcout << "Load Faces assigned to 102 = " << f102 << std::endl;
...
template <int dim>     
unsigned int tetElast<dim>::solve()
{
    PETScWrappers::MPI::Vector distributed_incremental_displacement(
      locally_owned_dofs, mpi_communicator);
    distributed_incremental_displacement = incremental_displacement;
    SolverControl solver_control( //35, 0.003);
    		dof_handler.n_dofs(),
                                1e-16 * system_rhs.l2_norm()) ;  // no forces used right now

    PETScWrappers::SparseDirectMUMPS spm(solver_control, mpi_communicator);
...
int main(int argc, char **argv)
{
  ...
  const std::string in_mesh_filename = "input/Flap_aq.msh"; // partial_flap_aq.msh"; //
  const std::string nodalbc_filename = "input/genfile.csv";
  const std::string out_mesh_filename = ("output/Flap_aq_mesh.vtk");
