                                                  ...
                                                  const double nu)
{
  ...
  tmp[1][0] = nu * Eprime;
  tmp[1][1] = Eprime;
  tmp[2][2] = 0; // could remove per 16.5-12
  tmp[3][3] = G;
  tmp[4][4] = 5*G/6;
  tmp[5][5] = 5*G/6;
  return tmp;
...
//  void nodal_bcs();
  ...
  std::vector<TemperatureData> temperatureRecords;
...
//  constraints.print(std::cout);
//  bogo();
  nodal_bcs<dim,spacedim>(nodalbc_filename ,  dof_handler   ,constraints);  // Applying the nodal constraints using WMU function
...
void shElast<dim, spacedim>::assemble_system()
{
  ...
  const unsigned int dofs_per_vertex = fe.n_dofs_per_vertex();
//  const unsigned int n_q_points =    quadrature_formula.size();
  ...
  int num_ptLoads = 3;  // There's a better way to handle point loads
  Vector<double>     ptld_rhs(num_ptLoads);
  std::vector<types::global_dof_index> ptld_idx(num_ptLoads);


  std::vector<double> zpt = {-0.774597, 0.0, 0.774597}; // sqrt(3/5)
  std::vector<double> zwt = {0.55555555555, 0.88888888888, 0.55555555555}; // 5/9 , 8/9

  // Coefficient of thermal expansion
  double alpha = 33e-6;  // 1/K
  double foo;
  double xsi;
  double eta;
  std::ofstream dbgf;
...
//			std::cout << "xsi: " << xsi << "   ,   eta: " << eta << std::endl;
			...
			{
				wmuSh.set_zeta(zpt[z]);
				wmuSh.set_J(xsi , eta, zpt[z]);
				wmuSh.calc_Jinv();
				wmuSh.calc_HJinv();
				wmuSh.set_N(xsi,eta);
				wmuSh.calc_B();

				dbgf << "----- Q_point = " << q_point << " -----"
						"----- Z_point = " << zpt[z] << std::endl;
				...
				if (pmatout	)
				{
					wmuSh.print_matrices(matout, xsi, eta);
					pmatout = false;
				}
...
//			std::cout << q_point << ":  aq_cell_matrix ------" << std::endl;
				aq_cell_matrix.triple_product(  Cmtx, wmuSh.B , wmuSh.B, true, false,
						wmuSh.detJ* quadrature_formula.weight(q_point) * zwt[z] );
	//					quadrature_formula.weight(q_point) );
...
//        std::cout << "--- Compare (0,0) Entries ---" << std::endl;
        dbgf << "V1, V2, V3 = " << std::endl << std::endl;
        dbgf << wmuSh.lmn1[0] << std::endl;
        dbgf << -wmuSh.lmn2[0] << std::endl;
        dbgf << wmuSh.corner_norms[0] << std::endl;
//        dbgf << "Deal.ii K matrix = " << std::endl << std::endl;
//        cell_matrix.print_formatted(dbgf, 1, true, 7, "   0   ", 1., 0.);
//        dbgf << "WMU method K matrix = " << std::endl << std::endl;
//        aq_cell_matrix.print_formatted(dbgf, 1, true, 7, "   0   ", 1., 0.);
...
//               constraints.print(std::cout);
               ...
               const Point<spacedim> load1(3., 3., 0.0);
               const Point<spacedim> load2(3., 0., 0.0);

//			   for (unsigned int v = 0; v<cell->n_vertices() ; v++)  // might be able to use 'vertex_iterator'
//				   {
//				   ptld_rhs(0)=0;
//				   ptld_idx[0]=0;
////				   bool apply_rhs = false;
//
//				   // Thermal load
//				   double dT = cell->vertex(v)[0];
//				   double TF = alpha * dT * 210e9 * 1 * wmuSh.cell_thick;
//				   ptld_rhs(0) = TF;   // assigns okay... but this could maybe be pre-assigned from file
//				   ptld_rhs(1) = TF;
//				   ptld_rhs(2) = TF;
//				   ptld_idx[0] = local_dof_indices[v*dofs_per_vertex+0];
//				   ptld_idx[1] = local_dof_indices[v*dofs_per_vertex+1];
//				   ptld_idx[2] = local_dof_indices[v*dofs_per_vertex+2];
//				   std::cout << "Load of " << ptld_rhs(0) << " applied to DOF " << ptld_idx[0] << std::endl;
//				   bool apply_rhs = true;
//
//
//				   if (apply_rhs)
...
//			                  constraints.print(std::cout);
  ...
  for (unsigned int dofno = 0; dofno < system_rhs.size() ; dofno++)
  {
	  if ( dofno % 6 < 2 ) // If an in-plane displacement dof
	  {
		  double dT = access_temp_data( 1+ floor(dofno / 6 ) );
		  double TF = alpha * dT * 210e9 * 1 * wmuSh.cell_thick;
		  system_rhs(dofno) += TF;
		  std::cout << "Thermal Force " << TF << " added to dof " <<  dofno << std::endl;
...
template <int dim, int spacedim>
void shElast<dim, spacedim>::load_temp_data()  // Chat GPT
{

	std::ifstream inputFile("input/temperature_data.txt");

	if (!inputFile) {
		std::cerr << "Error opening file: temperature_data.txt" << std::endl;
	   // return 1;
	}

	std::string line;
	std::getline(inputFile, line);   // Read and discard the header line ("Node,Temperature")

	while (std::getline(inputFile, line)) {
	        std::stringstream ss(line);
	        TemperatureData record;

	        // Assuming the format is "Node,Temperature"
	        char comma;
	        ss >> record.node >> comma >> record.temperature;
	...
	// Now, you have the data in the temperatureRecords vector.
	// You can process or display it as needed.

	// For example, let's display the data:
//	std::cout << "Node\tTemperature\n";
//	for (const auto& record : temperatureRecords) {
//		std::cout << record.node << "\t" << record.temperature << std::endl;
//	}
...
template <int dim, int spacedim>
double shElast<dim, spacedim>::access_temp_data(int tarnode)  // Chat GPT
{
	double dT = -273.0; // Default value in case Node  is not found

	    for (const auto& record : temperatureRecords)
	    {
	        if (record.node == tarnode)
	        {
	            dT = record.temperature;
	            break;
	        }
	    }
	    ...
	    if (dT != -273.0)
	    {
//	            std::cout << "Temperature at Node " << tarnode << ": " << dT << std::endl;
		} else
	        {
	            std::cout << "Node " << tarnode << " not found in the data." << std::endl;
	        }
...
unsigned int shElast<dim, spacedim>::solve()
{
	...
	std::ofstream dbgf;
	dbgf.open("output/Fullmatrices.txt");
	dbgf << std::endl << "=========================================" << std::endl;
	dbgf << "=========================================" << std::endl << std::endl;
	dbgf << "WMU RHS = " << std::endl << std::endl;
	        system_rhs.print(dbgf, 1, true);
	dbgf << std::endl << "WMU FULL K matrix = " << std::endl << std::endl;
			system_matrix.print_formatted(dbgf, 1, true, 3, " z ", 1.);
...
// 	umf.initialize(system_matrix);
// 	umf.vmult(solution, system_rhs);
// 	Vector<double> RHStoSolution;
// 	std::cout << "vmult" << std::endl;
// 	RHStoSolution = system_rhs;
// 	umf.solve(RHStoSolution);
// 	solution = RHStoSolution;
...
// 	umf.solve(system_matrix, solution, system_rhs);
...
int main(int argc, char **argv)
 {
  ...
  deallog.depth_console(2);
    ...
    const std::string in_mesh_filename = "input/reordflat3x3.inp";
    const std::string nodalbc_filename = "input/shell-bcs-load.csv";
    const std::string out_mesh_filename = ("output/thermal-demo_.vtu");
