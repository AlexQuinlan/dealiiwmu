...
//  std::vector<double> zwt = {0.55555555555/2, 0.88888888888/2, 0.55555555555/2}; // 5/9 , 8/9
  std::vector<double> zpt = {0.5};
  std::vector<double> zwt = {1};
...
//			std::cout << "xsi: " << xsi << "   ,   eta: " << eta << std::endl;
			for (unsigned int z = 0; z < zpt.size(); z++)
			{
...
//               constraints.print(std::cout);
			   ...
			   for (unsigned int v = 0; v<cell->n_vertices() ; v++)  // might be able to use 'vertex_iterator'
				   ...
				   if (cell->vertex(v).distance(load1) < 0.001)
					   {
					   ptld_rhs(0) = 1.e-0;   // assigns okay... but this could maybe be pre-assigned from file
					   ptld_idx[0] = local_dof_indices[v*dofs_per_vertex+2];
...
int main(int argc, char **argv)
 {
  ...
  deallog.depth_console(2);
    ...
    const std::string out_mesh_filename = ("output/aq-loadz-thin_2.vtu");
