                                          ...
                                          const unsigned int   q_point)
{
  ...
  for (unsigned int i = 0; i < spacedim; ++i)   // on the diagonal
  tmp[i][i] = fe_values.shape_grad_component(shape_func, q_point, i)[i];

  for (unsigned int i = 0; i < spacedim; ++i)   // off the diagonal
                                           ...
                                           const unsigned int   q_point)
 {
	 // Original
   SymmetricTensor<2, spacedim> tmp;
   SymmetricTensor<2, spacedim> iden;
   SymmetricTensor<2, spacedim> mult;
   std::cout << "shape function #: " << shape_func << std::endl;
   for (unsigned int i = 0; i < spacedim; ++i)   // on the diagonal
   {
	   tmp[i][i] = fe_values.shape_grad_component(shape_func, q_point, i)[i];
	   iden[i][i] = 1;
...
/*   Tensor J = get_Jacobian(cell_xyz, zeta, t)
		   Jacobians are related to the mapping
		   how can i associate thickness with the cell?

   // Calculate the inverse of the Jacobian at a given location
...
// {
//
//// what type is cell?  How can I extract the corner node coordinates from it?
//
// 	return 0.0;
// }
                                          ...
                                          const unsigned int   q_point)
{
  ...
  SymmetricTensor<2, dim> iden;
  SymmetricTensor<2, dim> mult;
...
//  ParameterHandler & parameters;
  ...
  static const SymmetricTensor<4, spacedim> shell_stress_strain_tensor;
...
template <int dim, int spacedim>
//const Tensor<2, 9> shElast<dim, spacedim>::H_matrix =
int shElast<dim, spacedim>::H2[6][9] = {0};
...
shElast<dim, spacedim>::shElast(
    ...
    const std::string &  nodalbc_filename,
    const std::string &  output_filename)
  : fe(FE_Shell<dim,spacedim>(1), 6) // 2nd arg changed to 6, # of elements AQ
    , dof_handler(triangulation)
    , quadrature_formula(fe.degree + 1)
    , initial_mesh_filename(initial_mesh_filename)
    , nodalbc_filename(nodalbc_filename)
    , output_filename(output_filename)
    , mapping(FE_Shell<dim,spacedim>(1))
//    , mpi_communicator(MPI_COMM_WORLD)
//    , n_mpi_processes(Utilities::MPI::n_mpi_processes(mpi_communicator))
//    , this_mpi_process(Utilities::MPI::this_mpi_process(mpi_communicator))
//    , std::cout(std::cout, this_mpi_process == 0)
...
//                           std::strtod(fields[1].c_str(), NULL), std::strtod(fields[2].c_str(), NULL) );
  ...
  int locdof;
   ...
   std::cout << "# of BCs (pt,mag):  (" << bcpoints.size() << ", " << nbcs.size() << ")" << std::endl;
...
//        	if (cell->vertex(v)[0]        	 < 0.1)
  ...
  std::cout << "# of unresolved BCs (pt,mag):  (" << bcpoints.size() << ", " << nbcs.size() << ")" << std::endl;
...
//                	std::cout << "Calling Sym Tensor" <<std::endl;
...
//                    const double foo = get_glob_coords( 0.4, 0.4, cell);
                    //                    So, I think these are the B matrix
//                	std::cout << "Sym Tensor called successfully" <<std::endl;
...
//					const SymmetricTensor<2, dim> eps_phi_i = no_strain(2);  // filler function
//					const SymmetricTensor<2, dim> eps_phi_j = no_strain(2);

//                    unsigned int eps_phi_i = 1, eps_phi_j = 1;
...
//                  double tmp11 = (eps_phi_i *            //
//                          shell_stress_strain_tensor * //
//                          eps_phi_j              //
//                          ) ;
//                  double tmp22 = fe_values.JxW(q_point);
...
// !!!!!!!!!!!!      Temporarily:  Set all zero diagonal values to 1
        for (const unsigned int i : fe_values.dof_indices())
      ...
      // From S18 ... all of this is based on there being multiple steps
...
////// void hexElast<dim, spacedim>::setup_quadrature_point_history()
////// void hexElast<dim, spacedim>::update_quadrature_point_history()

template <int dim, int spacedim>
void shElast<dim, spacedim>::check_H()
{
	// Temporary function
	std::cout << "H " << std::endl;
...
template <int dim, int spacedim>
void shElast<dim, spacedim>::set_H2()
{
//	int tmp[6][9];
...
int main(int argc, char **argv)
{
  ...
  deallog.depth_console(2);
    ...
    const std::string out_mesh_filename = ("output/shell3x3.vtu");
