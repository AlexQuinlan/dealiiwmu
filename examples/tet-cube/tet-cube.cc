...
{
  GridIn<dim>(triangulation).read("input/trial.msh");
...
void Step3<dim>::assemble_system()
{
  ...
  Point<dim> np1(0.0, 0.0, 0.0);
  Point<dim> np2(0.0, 1.0, 0.0);
  Point<dim> np3(1.0, 0.0, 0.0);
  Point<dim> np4(1.0, 1.0, 1.0);
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      // point check.  need to cycle between vertices, also
      if (cell->vertex(0).distance(np1) < 0.001)
      {
    		  zone_vtx1++;
    		  constraints.add_line(cell->vertex_dof_index(0,0, cell->active_fe_index() ));
    		  cell->vertex_dof_index(0,1, cell->active_fe_index() );
    		  cell->vertex_dof_index(0,2, cell->active_fe_index() );
      }
      if (cell->vertex(0).distance(np2) < 0.001)
      {
    		  zone_vtx2++;
      }
      if (cell->vertex(0).distance(np3) < 0.001)
            {
          		  zone_vtx3++;
            }
      if (cell->vertex(0).distance(np4) < 0.001)
		{
			  zone_vtx4++;
		}
  ...
  std::cout << "BC1 Zone Vertices:  " << zone_vtx1 << std::endl;
  std::cout << "BC2 Zone Vertices:  " << zone_vtx2 << std::endl;
  std::cout << "BC3 Zone Vertices:  " << zone_vtx3 << std::endl;

  std::cout << "Load Zone Vertices:  " << zone_vtx4 << std::endl;
  ...
  // Maybe try another interpolation. Need to set boundary id

//  std::map<types::global_dof_index, double> newBCs;
//
//
...
//
//  //Cmask? seems like it can only mask for x, y z
//   ComponentMask Cmask = fe.component_mask(0)
//  const IndexSet aq_dofs = DoFTools::extract_dofs(dof_handler, Cmask);

   // Block masks can also be used instead of ComponentMask
