        ...
        // found a cell
        {
          ...
          for (const unsigned int i : GeometryInfo<dim>::vertex_indices())
            if (vertex_indices.find(cells.back().vertices[i]) !=
                vertex_indices.end())
// AQ --  here we could put somthing to allow vertex vallues of 0 to pass
              // vertex with this index exists
              cells.back().vertices[i] =
                vertex_indices[cells.back().vertices[i]];
            else if (cells.back().vertices[i] ==	0)
            	cells.back().vertices[i] = 0;
            else
              {
...
namespace
{
  ...
  public:
    ...
    void
    read_in_abqinp(std::istream &in);  // AQ variation of read_in_abaqus
    void
    write_out_avs_ucd(std::ostream &out) const;
    void
    shell_write_out_avs_ucd(std::ostream &out) const; // AQ variation
  ...
  private:
    ...
    std::vector<std::vector<double>> cell_type_list; // AQ
                                   ...
                                   const bool apply_all_indicators_to_manifolds)
{
  ...
  abaqus_to_ucd.read_in_abqinp(in);  // AQ variation of read_in_abaqus

  std::stringstream in_ucd;
  abaqus_to_ucd.shell_write_out_avs_ucd(in_ucd);

  // Temporarily printing out the UCD stream
//  std::ofstream outFile;
//  outFile.open("UCD_stream.txt");
//  outFile << in_ucd.rdbuf();
//  outFile.close();
...
//  Above is the AQ modified version. Below is the original
//
//template <int dim, int spacedim>
//void
//GridIn<dim, spacedim>::read_abaqus(std::istream &in,
...
//		{"shell", ReferenceCells::Shell}  // AQ added
...
namespace
{
  ...
  unsigned int get_el_type_number(std::string &eltype)
  {
	  if (eltype  == "S3")
		  return 1;
		  ...
		  return 3;
	  	  ...
	  	  return 4;
	  // Need to add in other elements
	  // Need to adjust element order
  ...
  template <int dim, int spacedim>  // AQ modified version of read_in_abaqus
  void
  Abaqus_to_UCD<dim, spacedim>::read_in_abqinp(std::istream &input_stream)
  {
    // References:
    // http://www.egr.msu.edu/software/abaqus/Documentation/docs/v6.7/books/usb/default.htm?startat=pt01ch02.html
    // http://www.cprogramming.com/tutorial/string.html
      ...
      cont:
        ...
        if (line.compare("*HEADING") == 0 || line.compare(0, 2, "**") == 0 ||
        ...
        else if (line.compare(0, 8, "*ELEMENT") == 0)
          {
        	...
        	// AQ added: ID element Type
        	std::string eltype ;
        	unsigned int eltno; // number code for element type
    		bool shell = false;  // Turn on if shell found

        	{
        		// Remove whitespace from line
        		line.erase(std::remove_if(line.begin(), line.end(), ::isspace),
        				line.end());
        		const std::string before_type = "TYPE=";
        		const std::size_t idx         = line.find(before_type);

        		if (idx != std::string::npos)
        		{
        			eltype = line.substr(idx + before_type.size());
        		}
        	  // Check if there is any info after the element type

        		const std::size_t idxcom = eltype.find(","); // maybe this should be within above ()
        		if ( idxcom != std::string::npos)
        		{
        			eltype = eltype.substr(0,idxcom);
        			eltno = get_el_type_number(eltype);
        		}

        		if (eltype == "S4" || eltype == "S3")
        		{
        			shell = true;
        		}
...
//        	                  {
//        	                    from_string(eltype,
//        	                                line.substr(idx + before_type.size()),
//        	                                std::dec);
//        	                  }
        	}
            ...
            if (shell)  // Added by AQ
            {
            	// extract the number of nodes
            	const std::size_t sidx = eltype.find("S");
            	nvtx  =  stoi(eltype.substr(  sidx+1, sidx+2));
            	// 2nd order shells
            	if ( nvtx == 6 )
            		nvtx	= 3;
            	// !!! Temporarily changing 2nd order shells!!!
            	if ( nvtx == 8 || nvtx == 9)
            		nvtx	= 4;
            ...
            {
            	nvtx = GeometryInfo<dim>::vertices_per_cell;
            }

            const unsigned int n_data_per_cell = 1 + nvtx ;
  ...
  template <int dim, int spacedim>
    void
    Abaqus_to_UCD<dim, spacedim>::shell_write_out_avs_ucd(std::ostream &output) const
    {

	  // Modified version of write_out_avs_ucd.
	  //
	  // References:
      // http://www.dealii.org/developer/doxygen/deal.II/structGeometryInfo.html
      // http://people.scs.fsu.edu/~burkardt/data/ucd/ucd.html
      ...
      // AQ Alternative UCD stream output checker
      std::ofstream outF;
      outF.open("insitu_UCD_stream.txt");
      outF << "# Abaqus to UCD mesh conversion" << std::endl;
      outF << "# exported directly from shell_write_out_avs_ucd " << std::endl;
      // ========================================================
      // ASCII UCD File Format
      // The input file cannot contain blank lines or lines with leading blanks.
      // Comments, if present, must precede all data in the file.
      ...
      outF << node_list.size() << "\t" << (cell_list.size() + face_list.size())
                   << "\t0\t0\t0"  << std::endl;
...
//      output << "two";
      // Write out node numbers
      // Loop over all nodes  // Should be okay to leave unmodified
      for (const auto &node : node_list)
        {
          // Node number
    	  output << node[0] << " \t";
...
//          output << "\t" ;
          ...
          outF.setf(std::ios::scientific, std::ios::floatfield);
          ...
          for (unsigned int jj = 1; jj < spacedim + 1; ++jj)
            {
              ...
              {
                output << static_cast<double>(node[jj]) << "\t";
              	outF << static_cast<double>(node[jj]) << "\t";
              }
              else
              {
                output << 0.0 << "\t";
...
//      output << "end of nodes" << std::endl;
//      outF << "end of nodes" << std::endl;
      // Write out cell node numbers
      // Modified for shell element recognition
      for (unsigned int ii = 0; ii < cell_list.size(); ++ii)
        {
    	  ...
    	  output << ii + 1 << "\t" << cell_list[ii][0] << "\t"
                 << (dim == 2 ? "quad" : "hex") << "\t";  // Edit here for tri/tet

    	  outF << ii + 1 << "\t" << cell_list[ii][0] << "\t"
    	                   << (dim == 2 ? "quad" : "hex") << "\t";  // Edit here for tri/tet
          for (unsigned int jj = 1; jj < GeometryInfo<dim>::vertices_per_cell + 1;
               ++jj)
          {
        	  output << cell_list[ii][jj] << "\t";
        	  outF << cell_list[ii][jj] << "\t";
          }
          output << std::endl;
          outF << std::endl;

//          output << "end of one cell" << std::endl;


        }

//      output << "Finished with all cells" <<std::endl;
//      outF << "Finished with all cells" <<std::endl;
