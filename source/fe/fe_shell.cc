...
namespace internal
{
  namespace FE_Shell
  {
    namespace
    {
      // This function will need to be modified or duplicated
      // for alternative quadrature configurations. AQ
      std::vector<Point<1>>
      get_QGaussLobatto_points(const unsigned int degree)
      {
        if (degree > 0)
          ...
          {
            using FEQ = dealii::FE_Shell_Base<1, 1>;
...
// FE_Shell Contructor can take arguments of:
// int degrees
// Quadrature<1> & points

template <int dim, int spacedim>
FE_Shell<dim, spacedim>::FE_Shell(const unsigned int degree)
  : FE_Shell_Base<dim, spacedim>(
      TensorProductPolynomials<dim>(
        Polynomials::generate_complete_Lagrange_basis(
          internal::FE_Shell::get_QGaussLobatto_points(degree))),
      FiniteElementData<dim>(this->get_dpo_vector(degree),
                             1,  // AQ # of vector components of the element
                             degree,
                             FiniteElementData<dim>::H1), // Should this be H2
                             // for displacement and H1 for rotation?
      std::vector<bool>(1, false))  // What about this vector? Any need to modify?
{
  this->initialize(internal::FE_Shell::get_QGaussLobatto_points(degree));
}

// *** AQ Shell ***
// Quadrature set by get_QGaussLobatto_poins, which is a
// simple function that makes an instance of the
// QGaussLabatto class.  Other quadrature classes are
// available in quadrature_lib.cc


template <int dim, int spacedim>
FE_Shell<dim, spacedim>::FE_Shell(const Quadrature<1> &points)
  : FE_Shell_Base<dim, spacedim>(
      TensorProductPolynomials<dim>(
...
template <int dim, int spacedim>
std::string
FE_Shell<dim, spacedim>::get_name() const
{
  ...
  if (equidistant == true)
    {
      if (this->degree > 2)
        namebuf << "FE_Shell<" << Utilities::dim_string(dim, spacedim)
...
template <int dim, int spacedim>
void
FE_Shell<dim, spacedim>::convert_generalized_support_point_values_to_dof_values(
...
}  // AQ: I don't see a need to change this function



template <int dim, int spacedim>
std::unique_ptr<FiniteElement<dim, spacedim>>
FE_Shell<dim, spacedim>::clone() const
{
  return std::make_unique<FE_Shell<dim, spacedim>>(*this);
}  // AQ: I don't see a need to change this function
...
FE_Shell<dim, spacedim>::compare_for_domination(
  ...
  const unsigned int                  codim) const
{
  ...
  // & cell domination
  // ----------------------------------------
  if (const FE_Shell<dim, spacedim> *fe_q_other =
        dynamic_cast<const FE_Shell<dim, spacedim> *>(&fe_other))
    {
...
}  // AQ: this might need to be revisited once we start with mixed tri-quad meshes


// explicit instantiations
#include "fe_shell.inst"
