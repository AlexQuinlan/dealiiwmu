...
#include <deal.II/fe/fe_dgp.h> // is this needed?
#include <deal.II/fe/fe_dgq.h> // is this needed?
#include <deal.II/fe/fe_nothing.h> // is this needed?
#include <deal.II/fe/fe_pyramid_p.h> // is this needed?
#include <deal.II/fe/fe_q_base.h> // is this needed?
#include <deal.II/fe/fe_simplex_p.h> // is this needed?
#include <deal.II/fe/fe_simplex_p_bubbles.h> // is this needed?
#include <deal.II/fe/fe_tools.h>
#include <deal.II/fe/fe_wedge_p.h>

// New shell
#include <deal.II/fe/fe_shell_base.h>
...
struct FE_Shell_Base<xdim, xspacedim>::Implementation
{
  /**
  ...
  initialize_constraints(const std::vector<Point<1>> &,
                         FE_Shell_Base<1, spacedim> &)
  {
    // no constraints in 1d
  }  // Commented because we won't be using 1D.  Only 2D


  template <int spacedim>
  static void
  initialize_constraints(const std::vector<Point<1>> & /*points*/,
                         FE_Shell_Base<2, spacedim> &fe)
  {
  ...
  }  // Commented since we'll only be using 2D for the shells
};



template <int dim, int spacedim>
FE_Shell_Base<dim, spacedim>::FE_Shell_Base(
  ...
  : FE_Poly<dim, spacedim>(
     ...
     // std::vector<ComponentMask>(1, std::vector<bool>(2, true)))  // AQ mod
      std::vector<ComponentMask>(1, std::vector<bool>(1, true)))  // AQ mod
...
template <int dim, int spacedim>
void
FE_Shell_Base<dim, spacedim>::initialize(const std::vector<Point<1>> &points)
{ // where does this get called from? fe_shell.cc:L80 (length 2)
  Assert(points[0][0] == 0,
  ...
  const unsigned int q_dofs_per_cell =
    Utilities::fixed_power<dim>(q_degree + 1); // AQ dim -> spacedim
...
template <int dim, int spacedim>
void
FE_Shell_Base<dim, spacedim>::get_interpolation_matrix(
  const FiniteElement<dim, spacedim> &x_source_fe,
  FullMatrix<double> &                interpolation_matrix) const
{
  // go through the list of elements we can interpolate from
  if (const FE_Shell_Base<dim, spacedim> *source_fe =
        dynamic_cast<const FE_Shell_Base<dim, spacedim> *>(&x_source_fe))
    {
...
template <int dim, int spacedim>
void
FE_Shell_Base<dim, spacedim>::get_face_interpolation_matrix(
...
template <int dim, int spacedim>
void
FE_Shell_Base<dim, spacedim>::get_subface_interpolation_matrix(
  const FiniteElement<dim, spacedim> &source_fe,
...
template <int dim, int spacedim>
bool
FE_Shell_Base<dim, spacedim>::hp_constraints_are_implemented() const
{
  return true;
}
...
FE_Shell_Base<dim, spacedim>::hp_vertex_dof_identities(
  const FiniteElement<dim, spacedim> &fe_other) const
{
  if (dynamic_cast<const FE_Shell_Base<dim, spacedim> *>(&fe_other) != nullptr)
    {
...
template <int dim, int spacedim>
std::vector<std::pair<unsigned int, unsigned int>>
FE_Shell_Base<dim, spacedim>::hp_line_dof_identities(
  const FiniteElement<dim, spacedim> &fe_other) const
{
  // we can presently only compute these identities if both FEs are FE_Qs or
  // if the other one is an FE_Nothing
  if (const FE_Shell_Base<dim, spacedim> *fe_q_other =
        dynamic_cast<const FE_Shell_Base<dim, spacedim> *>(&fe_other))
    {
...
template <int dim, int spacedim>
std::vector<std::pair<unsigned int, unsigned int>>
FE_Shell_Base<dim, spacedim>::hp_quad_dof_identities(
...
template <int dim, int spacedim>
void
FE_Shell_Base<dim, spacedim>::initialize_unit_support_points(
...
template <int dim, int spacedim>
void
FE_Shell_Base<dim, spacedim>::initialize_unit_face_support_points(
  const std::vector<Point<1>> &points)
{
...
template <int dim, int spacedim>
void
FE_Shell_Base<dim, spacedim>::initialize_quad_dof_index_permutation()
{
...
template <int dim, int spacedim>
unsigned int
FE_Shell_Base<dim, spacedim>::face_to_cell_index(const unsigned int face_index,
...
FE_Shell_Base<dim, spacedim>::get_dpo_vector(const unsigned int degree)
{
  using FEQ = FE_Shell_Base<dim, spacedim>;
//  std::cout << "in FE_Shell_Base" << std::endl;
  AssertThrow(degree > 0, typename FEQ::ExcFEQCannotHaveDegree0());
  std::vector<unsigned int> dpo(dim + 1, 1U);

// Modified for shell
  // dpo[0] = 2;  // Vertex
  // dpo[1] = 2 * (degree-1);  // Line
  // dpo[2] = 2 * (degree-1);  // Face

// Original
  dpo[0] = 1;  // Vertex
  dpo[1] = 1 * (degree-1);  // Line
  dpo[2] = 1 * (degree-1);  // Face

//  for (unsigned int i = 1; i < dpo.size(); ++i)
//    dpo[i] = (dpo[i - 1] * (degree - 1)) * 2; // AQ times 2, dips and rot
  return dpo;

  // Mark for change.  This dpo vector will not follow
  // the FE_Q rules.  !!
}
...
template <int dim, int spacedim>
void
FE_Shell_Base<dim, spacedim>::initialize_constraints(
  const std::vector<Point<1>> &points)
{
...
FE_Shell_Base<dim, spacedim>::get_prolongation_matrix(
  ...
  if (this->prolongation[refinement_case - 1][child].n() == 0)
    {
      ...
      internal::FE_Shell_Base::zero_indices<dim>(j_indices);
      for (unsigned int j = 0; j < q_dofs_per_cell; j += dofs1d)
        {
          unsigned int i_indices[dim];
          internal::FE_Shell_Base::zero_indices<dim>(i_indices);
          for (unsigned int i = 0; i < q_dofs_per_cell; i += dofs1d)
            {
              ...
              internal::FE_Shell_Base::increment_indices<dim>(i_indices, dofs1d);
            }
          Assert(i_indices[dim - 1] == 1, ExcInternalError());
          internal::FE_Shell_Base::increment_indices<dim>(j_indices, dofs1d);
        }
...
template <int dim, int spacedim>
const FullMatrix<double> &
FE_Shell_Base<dim, spacedim>::get_restriction_matrix(
  const unsigned int         child,
  const RefinementCase<dim> &refinement_case) const
{
  AssertIndexRange(refinement_case,
              ...
              for (unsigned int j = 0; j < q_dofs_per_cell; j += dofs1d)
                {
                  ...
                  internal::FE_Shell_Base::increment_indices<dim>(j_indices,
                                                              dofs1d);
                }
...
template <int dim, int spacedim>
bool
FE_Shell_Base<dim, spacedim>::has_support_on_face(
...
FE_Shell_Base<dim, spacedim>::get_constant_modes() const
{
